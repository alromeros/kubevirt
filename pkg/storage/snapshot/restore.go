...
func (t *vmRestoreTarget) reconcileBackendVolume() (bool, error) {
	...
	// Retrieve only the backend volume for the snapshot
	...
	pvc, err := t.controller.getPVC(snapshotVM.Namespace, volumes[0].Name)
	if err != nil || pvc == nil {
		return false, err
	}

	// Update labels for the primary backend PVC if labels exist
	if pvc.Labels != nil {
	...
	// Update labels for the restore PVC if it matches the backend volume
	for _, vr := range t.vmRestore.Status.Restores {
		if vr.VolumeName == pvc.Name {
